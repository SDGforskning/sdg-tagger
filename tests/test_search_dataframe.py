...
def test_format_df_value_correct_formatting(
    input_value, input_sdg_nr, input_target, output_expected
):
    # Arrange
    # Act
...
def test_format_results_calls(mocker):
    ...
    expected_calls = [call(True, '1', '1'), call(False, '1', 'b')]
        ...
        (
            {
                ...
                '15': {'sdg_number': '15', 'targets': {'1': {'1': True}}},
            },
            [1, 15],
...
def test_format_results_including_df_value_formating(
    input_dict, input_sdgs, expected_output
):
    # Arrange
    # Act
...
def test_row_search(
    mock_format_results, mock_search_all_goals, input_text, input_sdgs, output_expected
):
    # Arrange
...
@pytest.mark.dataframe_search
@pytest.mark.parametrize('input_text, input_sdgs', [('Some text', [1])])
...
@pytest.mark.dataframe_search
@pytest.mark.parametrize(
    'input_data, input_column', [({'text': ['test123', 'test321']}, 'text')]
)
...
def test_dataframe_search(input_data, input_column):
    ...
    for sdg_nr in LIST_ALL_SDG_NR:
        for _, value in sdg_columns[sdg_nr]:
