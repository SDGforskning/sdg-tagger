...
1. Check you are on your branch (it is easiest to create a new branch online from the web version of the repository - for now, do this using initial_core_functionality branch as the source, _not_ "main" (default)). Then change to your new branch in VScode by clicking on the branch name (a tiny label in the bottom left corner), and choosing your new branch from the "remote" section of the list. As long as your branch name is in the bottom left corner, you are on your branch. 

    <img width="295" height="131" alt="bilde" src="https://github.com/user-attachments/assets/80f85927-c9d7-4d88-a144-974815341b1b" />

2. If converting an old SDG: Create a copy of the "template" file for your SDG (ctrl+c then ctrl+v). Rename it to `sdgx.json`, where 'x' is the number of the SDG. You will edit the new file (sdgx.json) you have just created by copying (not the original template). All the template files have the correct number of goals for that SDG, and the same number of phrases (but you will need to add more termlists, each phrase only has one in the template). If creating a new SDG...
...
5. The SDG must be complete before you test (if it has empty termlists/phrases, it will (probably?) not work in demo or pass py-tests). Once finished, run run_pytest.ipynb. Check that it says SUCCESS for the SDG (no FAILED). Click "open as a scrollable element" at the bottom of the output to see in more detail what failed. Failed means that something is not working with the JSON set up (a technical formatting error, using a termlist that hasn't been defined, etc).
6. Fix any technical errors _(! if just converting an old SDG, stop here !)_
7. Test your search using the functions in demo.ipynb. Here additional errors may also become issues when trying to run a search, such as missing brackets in the SDG.json files. See [Functions in demo for testing](#testing)
