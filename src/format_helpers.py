...
def _check_for_missing_matches(
    ...
    if not set(matches).issubset(set(all_logic_results)):
        message = f'''WARNING: The logic rule references searches that were not found! Make sure the logic rule only reference 
        termlists within the same phrase, pre-searches within the same file, or country searches. Searches not found:
        {', '.join(str(x) for x in set(matches) - set(all_logic_results))}'''
